...
const productsDb: ProductDetailItem[] = [
  {
    id: '1',
...
export const useProductDetail = () => {
  ...
  const handleAddToCart = () => {
    ...
    const existingIndex = cartList.findIndex((item) => item.cartItemId === cartItemId || item.id === product.id)
  ...
  const handleContactSupplier = () => {
    message.info(`正在为您接入【${product.supplier.name}】的专属招商服务代表，请稍后...`)
  }
